...
function handleClick(clickedButtonEvent) {
    ...
    let buttonText = clickedButtonEvent.currentTarget.textContent;
...
function hideMoreText(card) {
    ...
    let descClass = card.wrap.querySelector(".desc");
    descClass.remove();
...
function showMoreText(card) {
    ...
    let newText = "При клике по кнопке, вместо картинки над этой кнопкой появлялся какой-нибудь текст.//Completed";
    // 6.2 внутри .product есть .productName, после него вставляем div.desc и текстом из переменной из п. 6.1
    card.button.insertAdjacentHTML("beforebegin",`<div class="desc">${newText}</div>`);
